...
import { getTooltipContentInventoryTypeList } from '@/utils/tooltipInventoryTypeList'
...
interface tableData {
    ...
    inventoryTypeModels: InventoryTypeModel[];
}
...
const Page = () => {
    ...
    const [saveData, setSaveData] = useState<SaveTableData[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:8080/api/fspms/inventorytypelist?marshaCode=${marsha}`);
                const data = await response.json();
                console.log('Data fetched successfully:', data);

                if(response.ok){
                    const sortedData = data.inventoryTypeModels.sort((a: InventoryTypeModel, b: InventoryTypeModel) => a.inventoryCode.localeCompare(b.inventoryCode));
                    setTableData({ ...data, inventoryTypeModels: sortedData });
                        ...
                        // Initialize saveData with existing data
                        const initialSaveData = sortedData.map((item: { roomPool: any; inventoryCode: any; roomConfiguration: any; shoppableAttributes: any; capacity: any; baseOccupancy: any }) => ({
                            marshaCode: marsha,
                            roomPool: item.roomPool,
                            inventoryType: item.inventoryCode,
                            roomConfiguration: item.roomConfiguration,
                            currencyCode: 'USD', // Placeholder if needed
                            displayable: 'Yes', // Default or based on your needs
                            extraPersons: '',
                            minRate: '',
                            maxRate: '',
    ...
    const handleMinRateChange = (index: number, newValue: string) => {
        setSaveData(prevData => {
            const updatedData = [...prevData];
            updatedData[index] = { ...updatedData[index], minRate: newValue }; // Update minRate
            return updatedData;
        });
    }

    const handleMaxRateChange = (index: number, newValue: string) => {
        setSaveData(prevData => {
            const updatedData = [...prevData];
            updatedData[index] = { ...updatedData[index], maxRate: newValue }; // Update maxRate
            return updatedData;
        });
    }

    const handleExtraPersonChange = (index: number, newValue: string) => {
        setSaveData(prevData => {
            const updatedData = [...prevData];
            updatedData[index] = { ...updatedData[index], extraPersons: newValue }; // Update extraPersons
            return updatedData;
        });
    }
    ...
    const handleCommentSave = async () => {
        ...
        const pageName = 'Inventory Type List';
    ...
    const onSave = async () => {
        ...
        try{
            const response = await fetch(`http://localhost:8080/api/fspms/upload/inventortypelist?code=${marsha}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        <HotelProvider>
            ...
            <div className='flex justify-evenly mb-4'>
                    ...
                    <div className='flex mt-5'>
                        <div className='font-bold text-lg place-content-center'>INVENTORY TYPE LIST</div>

                        <div className='ml-4 grid place-content-center'>
                            <Tooltip text={getTooltipContentInventoryTypeList()}>
                    ...
                    <div className='flex justify-end mt-4 mb-6'>
                        ...
                        <select className='border border-gray-400 rounded-md ml-2 px-3'>
                            {tableData?.currencyCodes?.map((currencyCode, index) => (
                                <option key={`${currencyCode}-${index}`}>{currencyCode}</option>
                            ))}
                        </select>
                    </div>

                    <div className='overflow-y-auto max-h-[400px] overflow-x-auto border border-[#F5F5F5] rounded-lg'>
                        <table className=" w-full  bg-white border-collapse border border-[#F5F5F5]">
                            <thead className='text-xs'>
                                <tr className="bg-[#A80023] text-white   border-b border-[#F5F5F5]">
                                    ...
                                    <th className="px-6 py-3 border-r border-[#F5F5F5]">MIN RATE</th>
                                    <th className="px-6 py-3 border-r border-[#F5F5F5]">MAX RATE</th>
                                    <th className="px-6 py-3 border-r border-[#F5F5F5]">DISPLAYABLE</th>
                                    <th className='px-6 py-3 border-r border-[#F5F5F5]'>BASE OCCUPANCY</th>
                                    <th className="px-6 py-3 border-r border-[#F5F5F5]">EXTRA PERSON</th>
                            ...
                            <tbody>
                            {tableData?.inventoryTypeModels.map((row, index) => (
                                <tr key={index} className="border-b border-[#F5F5F5] text-center">
                                    <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">{row.inventoryCode}</td>
                                    <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">{row.capacity}</td>
                                    <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">{row.roomPool}</td>
                                    <td className="px-4 py-3 whitespace-nowrap border-r bg-white">
                                        <input
                                            type="text"
                                            value={saveData[index]?.minRate || ''}
                                            onChange={(e) => handleMinRateChange(index, e.target.value)}
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap border-r bg-white">
                                        <input
                                            type="text"
                                            value={saveData[index]?.maxRate || ''}
                                            onChange={(e) => handleMaxRateChange(index, e.target.value)}
                                            className="px-2 py-1 w-12"
                                        />
                                    </td>
                                    <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">Yes</td>
                                    <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">{row.baseOccupancy}</td>
                                    <td className="px-4 py-3 whitespace-nowrap border-r bg-white">
                                        <input
                                            type="text"
                                            value={saveData[index]?.extraPersons || ''}
                                            onChange={(e) => handleExtraPersonChange(index, e.target.value)}
                                            className="px-2 py-1 w-12"
                                        />
                                    </td>
                                    <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">{row.roomConfiguration}</td>
                                    <td className="px-4 py-3 text-left  whitespace-nowrap border-r bg-[#F5F5F5]">{row.shoppableAttributes.join(', ')}</td>
                                </tr>
                            ))}
                            </tbody>
                        </table>
