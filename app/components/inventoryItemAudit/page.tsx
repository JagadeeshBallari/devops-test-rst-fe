...
const Page = () => {
            ...
            if (response.ok) {
                if (contentType && contentType.includes('application/json')) {
                    ...
                    if(responseExisting.ok) {
                        ...
                        const initialSelectedOptions = data.map((row: { shoppable: string[]; }) => {
                            return row.shoppable.map((description: string) => ({
                                value: description,
                                label: description
                            }));
                        });
    ...
    const handleConnecting1Change = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
        const value = event.target.value;
        setTableData(prevData =>
          prevData.map((item, i) =>
            i === index ? { ...item, connectingRoom1: value } : item
          )
        );
      };

      const handleConnecting2Change = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
        const value = event.target.value;
        setTableData(prevData =>
          prevData.map((item, i) =>
            i === index ? { ...item, connectingRoom2: value } : item
          )
        );
      };

      const handleConnecting3Change = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
        const value = event.target.value;
        setTableData(prevData =>
          prevData.map((item, i) =>
            i === index ? { ...item, connectingRoom3: value } : item
          )
        );
      };
    
    const exportToExcel = (data: TableData[]) => {
        ...
        link.download = `${marsha}-Physical Audit.xlsx`;
    ...
    const handleSave = async () => {
        if (!marsha) {
            console.error('Marsha is not set.');
            return;
        }
        try {  
            const dataWithMarsha = tableData.map(item => ({ ...item, marshaCode: marsha }));
            const response = await fetch(`http://localhost:8080/api/fspms/inventoryaudit?code=${marsha}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify(dataWithMarsha),
            });

            if (response.ok) {
                console.log(dataWithMarsha)
    ...
    const handleCommentSave = async () => {
        ...
        const pageName = 'Inventory Audit';
    ...
    <>
        ...
        <HotelProvider>
            ...
            <div className='flex mt-5 '>
                <div className='font-bold text-lg  place-content-center'>PHYSICAL GUESTROOM AUDIT</div>
            ...
            <div className='flex justify-end mt-4 mb-6'>
                ...
                <button className="font-semibold border border-[#A80023]  px-2 py-1 text-[#A80023] center rounded" onClick={() => exportToExcel(tableData)}>
                    <div className='flex'>
                        <div className='place-content-center'><Image src='/download.png' alt='Logo' width={24} height={24}/></div>
                        <div className=''>EXPORT FILE</div>
                    </div>
                </button>
            </div>
                ...
                <table className=" min-w-full  bg-white border-collapse border border-[#F5F5F5]">
                    <thead className='text-xs'>
                        <tr className="bg-[#A80023] text-white  border-b border-[#F5F5F5]">
                            ...
                            <th className="px-6 py-3 border-r border-[#F5F5F5]">BUILDING</th>
                            <th className="px-6 py-3 border-r border-[#F5F5F5]">FLOOR</th>
                            <th className='px-6 py-3 border-r border-[#F5F5F5]'>ROOM POOL</th>
                            <th className="px-6 py-3 border-r border-[#F5F5F5]">ROOM SIZE</th>
                    ...
                    {tableData.map((item, index) => {
                        const key = item.room ? `${item.room}-${index}` : `item-${index}`;

                        return (
                            <tr key={key} className="border-b border-[#F5F5F5] text-center">
                                ...
                                <td className="px-4 py-3 whitespace-nowrap border-r bg-white">
                                    <input
                                        type="text"
                                        value={item.connectingRoom1}
                                        className="w-full rounded-md px-4 py-2"
                                        onChange={e=>handleConnecting1Change(e, index)}
                                    />
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap border-r bg-white">
                                    <input
                                        type="text"
                                        value={item.connectingRoom2}
                                        className="w-full rounded-md px-4 py-2"
                                        onChange={e=>handleConnecting2Change(e, index)}
                                    />
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap border-r bg-white">
                                    <input
                                        type="text"
                                        value={item.connectingRoom3}
                                        className="w-full rounded-md px-4 py-2"
                                        onChange={e=>handleConnecting3Change(e, index)}
                                    />
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">{item.buildingNo}</td>
                                <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">{item.floorNo}</td>
                                <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">{item.roomPool}</td>
                                <td className="px-4 py-3 whitespace-nowrap border-r bg-[#F5F5F5]">{item.roomSize}</td>
